...
export class ChainsService {
  ...
  getAvailableCurrencies = async (args: {
    ...
    // aggregate currenceis from origin and target chains
    const currencies = this.getChains(process.env.KIMA_ENVIRONMENT as ChainEnv)
      .filter((chain) => [originChain, targetChain].includes(chain.shortName))
      .map((chain) => chain.supportedTokens)
      .map((tokens) => tokens.map((token) => token.symbol))
      .flat()
    return currencies
