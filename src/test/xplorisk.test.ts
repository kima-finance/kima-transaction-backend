...
describe('xplorisk API test', () => {
  it('should reject empty addresses', async () => {
    await expect(getRisk([])).rejects.toThrowError('Must provide at least one address to check');
  });

  it('should get an address with low risk', async () => {
    const results: Array<RiskResult> = await getRisk(['0x47B079c8EE493678e771886103705b39BF1A4992']);
    expect(results.length).toBe(1);
    expect(results[0].risk_score).toEqual(RiskScore.LOW);
  });

  it('should get an address with medium risk', async () => {
    const results: Array<RiskResult> = await getRisk(['0x001474b877f98f41701397a65d4d313ab180c7b2']);
    expect(results.length).toBe(1);
    expect(results[0].risk_score).toEqual(RiskScore.MED);
  });

  it('should get an address with high risk', async () => {
    const results: Array<RiskResult> = await getRisk(['0x39D908dac893CBCB53Cc86e0ECc369aA4DeF1A29']);
    expect(results.length).toBe(1);
  ...
  it('should get multiple results', async () => {
    ...
    const totalRisky: number = results.reduce((a, c) => a + (c.risk_score !== RiskScore.LOW ? 1 : 0), 0);
